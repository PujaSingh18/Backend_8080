...
//payhist
const payhistController = require("../controllers/payhistController");
...
//Route to Pay Hist
router.post("/create/payhist", payhistController.createRecord);
router.get("/get/payhist/:id", payhistController.getRecordById);
router.get("/get/payhist", payhistController.getAllRecords);
router.put("/update/payhist/:id", payhistController.updateRecordById);
router.delete("/delete/payhist/:id", payhistController.deleteRecordById);
...
router.post('/retailer-login', async (req, res) => {
  ...
  try {
      // Query the database to check if retailer exists
      const [rows] = await pool.query('SELECT * FROM users WHERE mobile_number = ? AND name = ? AND user_type = ?', [mobileNumber, name, 'Retailer']);
      console.log('Retailer login query result:', rows);
      if (rows.length === 0) {
          // Retailer not found
          return res.status(404).json({ message: 'Retailer not found' });
      }
      // Retailer found, return success response
      return res.status(200).json({ message: 'Retailer logged in successfully', user: rows[0] });
  } catch (error) {
